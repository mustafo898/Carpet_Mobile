...
class ProductRepository @Inject constructor(private var service: ApiService) {
    suspend fun getPagination(
        ...
        size: Int,
        type: String
    ): LiveData<BaseNetworkResult<List<ProductPaginationResponse>>> {
    ...
    suspend fun productDetails(
        id: String,
        type: String
    ): LiveData<BaseNetworkResult<ProductResponse>> {
        ...
        if (response.code() == 200) {
            ...
            list.value = BaseNetworkResult.Loading(false)
            Log.d("Error", "productDetails: ${response.message()}")
            list.value = BaseNetworkResult.Error(response.message())
        }
        return list
    }
    ...
    suspend fun updateProduct(
        id: String,
        type: String,
        productUpdate: ProductCreateRequest
    ): LiveData<BaseNetworkResult<ProductResponse>> {
        val list = MutableLiveData<BaseNetworkResult<ProductResponse>>()
    ...
    suspend fun fileUploadProduct(
        productId: String,
        file: MultipartBody.Part
    ): LiveData<BaseNetworkResult<ProductFileUploadResponse>> {

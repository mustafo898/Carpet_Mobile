...
class UpdateUserViewModel @Inject constructor(private val profileUseCase: ProfileUseCase) :
    ViewModel() {
    ...
    fun update(
        ...
        viewModelScope.launch {
            if (!validName(name) && !validPhone(number) && !validPassword(password) && !validSurname(
                    surname
                )
            ) {
                validName(name)
                ...
                profileUseCase.updateUsersProfile(
                    id,
